...
\begin{frame}{実験評価計画 (1/2)}
    \begin{itemize}
        \item 7月
        \begin{itemize}
            \alert{\item 各評価指標の意味を理解}
            \alert{\item 具体的な目標精度を決定}
            \item 研究計画書を作成
        \end{itemize}
        \item 8月
        \begin{itemize}
...
%     vgrid,
%     hgrid,
%     bar/.append style={fill=cyan!80},
%     % 'incomplete'スタイルが 'bar' のスタイルに追記されるように定義
%     incomplete/.style={bar/.append style={fill=orange!80}},
...
%   % タスクリスト
%   % 'incomplete'スタイルを直接指定
%   \ganttbar[incomplete]{評価指標の理解・目標精度決定}{1}{1} \\
%   \ganttbar{研究計画書の作成}{1}{1} \\
%   \ganttbar{モデル/学習戦略の検討}{2}{2} \\
%   \ganttbar{精度への影響調査}{3}{3} \\
%   \ganttbar{連合学習モデルの精度比較}{3}{4}
